...
<script>
	$(document).ready(function () {
		...
		$('.view_receipt').click(function () {
			...
			var nw = window.open(_base_url_ + "cashier/sales/print_receipt.php?id=" + $(this).attr('data-id'), '_blank', "width=" + ($(window).width() * .2) + ",left=" + ($(window).width() * .3) + ",height=" + ($(window).height() * .8) + ",top=" + ($(window).height() * .1))
			setTimeout(() => {
				nw.print()
				setTimeout(() => {
					nw.close()
