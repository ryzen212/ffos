                        ...
                        <div id="menu-list" class='col-xl-8 col-lg-12 col-md-12 col-sm-12'>
                            ...
                            <div class="row">
                                <button
                                    ...
                                    type='button' id='add_order' data-toggle="tooltip" data-placement="top"
                                    title="Add Order">
            ...
            <form id='customer_frm'>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group mt-2">
                        ...
                        <input type="text" class="form-control" name='queue' id="queue">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary" id='customer_submit'>Submit</button>
                </div>
            </form>
        </div>
    </div>
...
<script
    src="<?php echo base_url ?>/plugins/qr_scanner/cdnjs.cloudflare.com_ajax_libs_html5-qrcode_2.3.4_html5-qrcode.min.js"
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('.variant_btn').click(function () {
            ...
            $.ajax({
                url: _base_url_ + "cashier/sales/var_modal.php",
                type: "post",
                data: { id: id },
        ...
        $('.item-btn').click(function () {
            ...
            item.find('.rem-item').on("click", function () {
        ...
        $('#sales-form').submit(function (e) {
            ...
            var customer_queue = $('#queue').val()

            var data = $(this).serialize();
            data += '&customer_code=' + customer_code;
            data += '&customer_queue=' + customer_queue;
            ...
            $.ajax({
                ...
                success: function (resp) {
                    if (resp.status == 'success') {
                        alert_toast(resp.msg, 'success')
                        window.open(_base_url_ + "cashier/sales/receipt.php?id=" + resp.oid + '&discount=' + isDiscount, '_self')
        ...
        $('#add_order').click(function () {
            ...
            if (scanner === null) {
                // Scanner is not active, start the scanner
                ...
                // Show the modal after initializing the scanner
            ...
            function success(result) {
                ...
                //   alert(result.queue)
                document.getElementById('code').value = result.customer_code;

                document.getElementById('queue').value = result.queue;
                document.getElementById('customer_submit').click();

                scanner.clear();
                scanner.stop();


                // Additional actions after a successful scan
            }
        });
       

        $('#customer_frm').submit(function (e) {
            e.preventDefault()
            var data = $(this).serialize();
            // alert(data)
            // alert($('#menu-note').val())
            ...
            start_loader()
            $.ajax({
                url: _base_url_ + "classes/Master.php?f=get_customer_qrcode",
                method: 'POST',
                data: data,
                dataType: 'json',
                error: err => {
                ...
                success: function (resp) {
                    if (resp.status == 'success') {
                        ...
                        for (var i = 0; i < resp.data.length; i++) {
                            (function () {
                                var id = resp.data[i]['menu_id'];
                                var name = resp.data[i]['item'];
                                if (resp.data[i]['isVariants'] == true) {
                                    name = resp.data[i]['item'] + ' : ' + resp.data[i]['var_name'];
                                    id = resp.data[i]['menu_id'] + '-' + resp.data[i]['variants'];
                                }
                                var price = resp.data[i]['price'];
                                var code = resp.data[i]['code'];

                                var qty = resp.data[i]['quantity'];
                                var item = $($('noscript#item-clone').html()).clone()
                                if ($('#order-items-body .product-item[data-id="' + id + '"]').length > 0) {
                                    item = $('#order-items-body .product-item[data-id="' + id + '"]')
                                    var qty = resp.data[i]['quantity'];
